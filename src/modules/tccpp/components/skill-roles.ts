...
export default class SkillRoles extends BotComponent {
    ...
    override async setup(commands: CommandSetBuilder) {
        ...
        for (const id of skill_role_ids) {
            role_manager.register_role_update_listener(id, this.on_skill_role_change.bind(this));
        }
    }
    ...
    override async on_ready(): Promise<void> {
        this.skill_roles = await Promise.all(
            skill_role_ids.map(async (id, index) => {
                const role = await this.wheatley.TCCPP.roles.fetch(id);
    ...
    private async on_skill_role_change(role_id: string, member: Discord.GuildMember) {
