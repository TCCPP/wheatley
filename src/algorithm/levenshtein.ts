...
function find_min_action(actions: [number, number][]) {
    ...
    for (const action of actions) {
        if (action[0] < min[0]) {
            // closest
            min = action;
        } else if (action[0] == min[0] && action[1] < min[1]) {
            // fewest substitutions up until this point
            min = action;
        }
    }
    return min;
...
export function weighted_levenshtein_raw(src: string, target: string) {
    ...
    const d = new Array(src.length + 1).fill(0).map(() => new Array(target.length + 1).fill(0).map(() => [0, 0]));
    ...
    for (let i = 1; i <= src.length; i++) {
        for (let j = 1; j <= target.length; j++) {
            ...
            const ic = i == 0 || i == src.length ? 0.1 : insertion_cost;
    ...
    d[src.length][target.length][0] =
        d[src.length][target.length][0] -
        d[src.length][target.length][1] +
...
export function levenshtein(src: string, target: string) {
    // Stores tuples [distance, substitution_count]
    const d = new Array(src.length + 1).fill(0).map(() => new Array(target.length + 1).fill(0) as number[]);
    for (let i = 1; i <= src.length; i++) {
        d[i][0] = i;
    }
    for (let j = 1; j <= target.length; j++) {
    ...
    for (let i = 1; i <= src.length; i++) {
        for (let j = 1; j <= target.length; j++) {
            ...
            d[i][j] = Math.min(d[i - 1][j] + 1, d[i][j - 1] + 1, d[i - 1][j - 1] + ts);
        }
    }
    return d[src.length][target.length];
}
