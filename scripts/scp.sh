...
rsync -av --checksum package.json package-lock.json start.sh run-persist.sh tsconfig.json indexes src test wiki config.json scripts x0:projects/wheatley --exclude={"indexes/cppref/*.txt","indexes/cppref/*cppreference*","*.js","src/wheatley-private/.git"}
